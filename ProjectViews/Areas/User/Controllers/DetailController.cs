    ...
    public class DetailController : Controller
    {
        ...
        public async Task<IActionResult> DetailProduct(string name)
        {
            ...
            foreach (var item in gourpList)
            {
                ...
                shoe.ColorValue = "#" + colors.FirstOrDefault(p =>
                    p.Id == color_shoeDetails.FirstOrDefault(p => p.IdShoeDetail == item.Id).IdColor).ColorName;
                shoe.SizeValue = sizes.FirstOrDefault(p =>
                    p.Id == size_shoeDetails.FirstOrDefault(p => p.IdShoeDetails == item.Id).IdSize).SizeNumber;
                //check image null
